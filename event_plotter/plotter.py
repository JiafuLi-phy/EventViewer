...
def plot_peaks(
    ...
    if has_waveform(peaks):
        ...
        ax.set_ylabel("Amplitude [ADC/ns]" if to_pe is None else "Amplitude [PE/μs]")
        style.tighten_ylim(ax)
    else:
...
def plot_event_full(
    ...
    ax_pmt_top = fig.add_subplot(gs[2, 0])
    ax_pmt_bot = fig.add_subplot(gs[2, 1])

    fig._peak_axes = [ax_wf, ax_wf_log]
    ...
    if len(peaks):
        ...
        _draw_3layer_waveform(
            peaks, t0=t0_ev, ax=ax_wf_log, show_largest=show_largest,
            raw_records=raw_records, to_pe=to_pe, pmt_positions=pmt_positions,
            highlight_idx=highlight_peak_idx,
        )
    ...
    for ax_pmt, arr_name in [(ax_pmt_top, "top"), (ax_pmt_bot, "bottom")]:
        ...
        if "s2_x" in event.dtype.names and "s2_y" in event.dtype.names:
            ...
            if abs(s2x) > 0.01 or abs(s2y) > 0.01:
                ax_pmt.plot(s2x, s2y, 'rx', markersize=16, markeredgewidth=4,
                           zorder=10, label='S2')
...
def plot_peak_zoom(
    ...
    if has_waveform(np.array([peak])):
        ...
        ax_wf.set_ylabel("Amplitude [PE/μs]")
